...
export default function SpiralPostsChart() {
  const { data = {} } = useGetPostsCount()
  console.log("🚀 ~ SpiralPostsChart ~ data:", data)
  ...
  const postsData = [
    {
      ...
      fill: "blue",
      percentage: totalPosts > 0 ? Math.round(((countMap["FREE"] || 0) / totalPosts) * 100) : 0,
    },
    {
      type: "SILVER",
      posts: countMap["SILVER"] || 0,
      fill: "#C0C0C0",
      percentage: totalPosts > 0 ? Math.round(((countMap["SILVER"] || 0) / totalPosts) * 100) : 0,
    },
    {
      type: "GOLD",
      posts: countMap["GOLD"] || 0,
      fill: "#FFD700",
      percentage: totalPosts > 0 ? Math.round(((countMap["GOLD"] || 0) / totalPosts) * 100) : 0,
    },
    {
      type: "DIAMOND",
      posts: countMap["DIAMOND"] || 0,
      fill: "#B9F2FF",
      percentage: totalPosts > 0 ? Math.round(((countMap["DIAMOND"] || 0) / totalPosts) * 100) : 0,
    },
  ]
    ...
    <div className="px-2 mx-auto w-full max-w-4xl md:mx-0 md:p-6">
      <Card>
        <CardHeader className="text-center">
          <CardTitle className="text-2xl font-bold">Posts Distribution</CardTitle>
          <CardDescription>Total Posts: {totalPosts.toLocaleString()} across all tiers</CardDescription>
        </CardHeader>
        <CardContent>
          <ChartContainer config={chartConfig} className="mx-auto aspect-square max-h-[400px]">
            <RadialBarChart data={postsData} startAngle={90} endAngle={450} innerRadius={30} outerRadius={140}>
              {/* <ChartTooltip
                cursor={false}
                content={
                  <ChartTooltipContent
          ...
          <div className="p-4 mt-6 bg-gray-50 rounded-lg">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm md:grid-cols-4">
              <div className="text-center">
                <div className="font-bold text-gray-600">{postsData[0].posts}</div>
                <div className="text-gray-500">Free Posts</div>
              </div>
              <div className="text-center">
                <div className="font-bold text-gray-400">{postsData[1].posts}</div>
                <div className="text-gray-500">Silver Posts</div>
              </div>
              <div className="text-center">
                <div className="font-bold text-yellow-600">{postsData[2].posts}</div>
                <div className="text-gray-500">Gold Posts</div>
              </div>
              <div className="text-center">
                <div className="font-bold text-blue-400">{postsData[3].posts}</div>
